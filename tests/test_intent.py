...
class TestBuildTargetDescription:
    ...
    def test_generic_label_uses_selector_hint(self):
        elem = _make_elem(label="a", name=None, placeholder=None)
        assert _build_target_description(elem) == "test"

    def test_toggle_expected_effect_is_outcome_based(self):
    ...
    def test_close_glyph_maps_to_close_button_target(self):
        elem = _make_elem(label="×", aria_label=None, name=None, placeholder=None)
        assert _build_target_description(elem) == "close button"

    def test_h3_button_is_inferred_as_dismiss_intent(self):
        elem = _make_elem(
            label="",
            selector="#login_button_container > div > form > div:nth-of-type(3) > h3 > button",
            aria_label=None,
            name=None,
        )
        intent = infer_intent(elem, "click")
    ...
    def test_theme_toggle_target_is_inferred_from_dom_id(self):
        elem = _make_elem(
