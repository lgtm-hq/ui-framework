...
class TestContextStore:
    ...
    def test_dedup_across_extractions(self):
        store = ContextStore()
...
class TestSearchQuery:
    ...
    def test_rotation_no_repeats(self):
        store = ContextStore()
        store.extract_from(
            _make_analysis(entities=["A", "B", "C"])
        )
        seen = set()
        for _ in range(3):
    ...
    def test_prefers_listing_entities(self):
        store = ContextStore()
...
class TestFilterValue:
    ...
    def test_returns_untried_option(self):
        store = ContextStore()
        val = store.get_filter_value(["Action", "Comedy", "Drama"])
        assert val == "Action"

    def test_skips_tried_options(self):
        store = ContextStore()
