...
| Command used  | See commands below                                                |
| Commit SHA    | `8d02ee4`                                                         |
...
| -------------------- | ----------- | ---------------------- | --------------------------- |
| Runtime duration (s) | <= 1800     | 38.87                  | `flowscout benchmark`       |
| States discovered    | n/a         | 20                     | `flowscout benchmark`       |
| Actions executed     | n/a         | 22                     | `flowscout benchmark`       |
| Coverage percent     | >= 80%      | 100% page (36% action) | `flowscout benchmark`       |
| Low-confidence       | trend down  | 1 (<0.60 threshold)    | `flowscout benchmark`       |
| Flaky action count   | trend down  | 1                      | `benchmark` + `reliability` |
| Tests created        | > 0         | Yes (3 suites)         | run directory artifacts     |
| Test pass rate       | >= baseline | 362/362 passed         | `pytest --cov`              |

## Reliability Trend Review (Repeated Runs)

| Run                   | Dur (s) | States | Actions | Act Cov | Avg Conf | Low Conf | Flaky |
| --------------------- | ------: | -----: | ------: | ------: | -------: | -------: | ----: |
...
- One flaky action is now identified: click on the search form container
  produced both `no_change` and `navigation` outcomes across runs.
